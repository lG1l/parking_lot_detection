...
업로드 완료 알림 → 백엔드가 1단계 작업을 SQS에 넣음
  ① 정지 장면(시작 장면 1장) 추출 → 비식별화 → S3 마스킹본 영역   ← 가장 먼저
...
[확인]
후보마다 "찾음" → 사건 지점 고정
묶음 아래 [다음] (= 묶음 전체 "못 찾음", D-37) → 백엔드가 다음 묶음 작업을 SQS에 넣음 → 다음 10개 후보 클립 비식별화
```
...
| 정지 장면 | 업로드 직후 바로 입력하려는 경우 정지 장면 처리 시간만 기다린다. 첫 프레임 1장이라 시크 없이 끝난다 (D-48) |
...
| S3 (비공개 버킷 3개) | 원본, 마스킹본, 분석 결과를 버킷으로 분리 (D-23). **세 버킷 모두 수명 주기 규칙 30일 삭제**(파기 안전망, D-52·D-57)와 **HTTP 접근 거부 정책**(D-55). **버전 관리는 켜지 않는다**(켜면 삭제가 복원 가능해져 고시 제13조에 어긋난다, D-57) | 🔐 원본 버킷 CORS 설정, 수명 주기 규칙, 버킷 정책 설정 포함 |
...
- **리전은 서울 `ap-northeast-2`로 고정한다 (D-54, ⚠️ 팀장 확인).** GPU 인스턴스와 사용할 Bedrock 모델을 모두 이 리전에서 쓸 수 있어야 한다(모델 선택 조건, U-08).
...
- ⚠️ PRD 3.4: 원본 영상을 Bedrock으로 보내도 되는지 팀 확인이 필요하다. 안 되면 VLM 입력 설계를 다시 해야 한다. (C-01. 국외 이전 문제는 D-54로 없어지고, 남는 것은 위탁 범위의 문제다)
...
| 4 | S3 | **비공개 버킷 3개**(원본·마스킹본·분석 결과) 생성, 객체 읽기·쓰기, **원본 버킷 CORS 설정**(브라우저에서 S3로 직접 업로드), **버킷 3개의 수명 주기 규칙**(`s3:PutLifecycleConfiguration`, 30일 삭제), **버킷 3개의 버킷 정책 설정**(`s3:PutBucketPolicy`, HTTP 접근 거부). 버킷 수에는 요금이 없다. 계정 버킷 수 한도와 쓸 이름 확인 ⚠️ (C-08) | 영상 저장. 원본 접근을 IAM으로 차단. 파기는 법정 의무다 (D-52, D-57). 평문 전송 차단은 고시 제7조④ (D-55) | D-09, D-18, D-23, D-52, D-55, D-57 |
...
| `s3:DeleteObject` | 원본 영역 | 원본 파기 주기 작업 (D-52). **읽기가 아니라 삭제 권한이라 불변 조건 1을 깨지 않는다** |
| `s3:DeleteObject`, `s3:ListBucket` | 마스킹본·분석 결과 영역 | 마스킹본·분석 결과 파기 주기 작업 (D-57). `videos/{video_id}/` 아래를 훑어 지우므로 목록 조회가 함께 필요하다 |
