...
- S3의 세 영역을 버킷으로 나눌지 prefix로 나눌지는 🚧 (U-05)
...
- **2단계 시작 조건**: 1단계가 끝났고 입력이 있어야 한다. 누가 먼저 끝나든 자동으로 시작한다.
  ...
  - 두 경우가 겹쳐 2단계가 두 번 시작되지 않도록 DB 상태로 막는다. (🚧 04장, 06장)
- 진행 상황은 알림(이메일·문자) 없이, 로그인하면 화면에 상태로 보여준다. 이메일 발송(SES)은 추가 권한이 필요하고 데모에 필요하지 않다.
- 상태 이름(예: "대기 중 (앞에 N건)", "1단계 분석 중 (입력 가능)", "입력 완료·분석 중", "확인 가능", "실패")과 전이 규칙은 🚧 (04장)
...
| S3 (비공개 버킷) | 원본, 마스킹본, 분석 결과 | 🔐 버킷 CORS 설정 포함 |
...
| 4 | S3 | 비공개 버킷 생성(개수 🚧 U-05), 객체 읽기·쓰기, **버킷 CORS 설정**(브라우저에서 S3로 직접 업로드) | 영상 저장 | D-09, D-18 |
...
- HTTPS를 적용하지 않는다. 로그인 정보가 암호화되지 않은 채 전송된다. HTTPS에는 도메인과 인증서가 필요해 데모 범위에서 제외했다.
