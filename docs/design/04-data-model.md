...
| `upload_token_id` | bigint FK → upload_tokens | 어느 링크로 올라왔는지. 비어 있으면 주인이 직접 올린 것이다 (D-28) |
| `s3_key` | text | 원본 버킷 안의 키 (4.9) |
| `original_filename` | text | 관리자가 올린 파일 이름 |
| `size_bytes` | bigint | 업로드 완료 알림에서 받는다 |
...
차량 선택용 정지 장면. 원본 영상 중간 시점과, 거기서 5분씩 이전으로 이동한 시점이다. (PRD 3.2)

| 필드 | 타입 | 설명 |
|---|---|---|
| `id` | bigserial PK | |
| `analysis_job_id` | bigint FK → analysis_jobs | |
| `t_sec` | double precision | 원본 시간. `(analysis_job_id, t_sec)`에 UNIQUE |
| `masked_s3_key` | text | 비식별화가 끝난 이미지 키 (4.9) |
| `detections` | jsonb | 탐지된 차량 목록. 사용자가 드래그한 자리의 누끼를 확정하는 데 쓴다 (D-29) |
| `created_at` | timestamptz | |

- 원본 이미지는 저장하지 않는다. 비식별화된 것만 남긴다. (불변 조건 2)
- `detections` 예시 (형식은 06장에서 확정 🚧):
  ```json
  [{"det_id": 1, "bbox": [810, 440, 960, 620], "conf": 0.91,
    "polygon": [[812, 441], [958, 445], [956, 618], [814, 615]]}]
  ```
- `polygon`은 누끼(차량 외곽)다. 화면에 선택된 차량을 또렷하게 표시하는 데 쓴다. 외곽을 딸지 사각형만 쓸지는 🚧 (06장, U-09). 없으면 `bbox`로 대신한다.
...
| `still_frame_id` | bigint FK → still_frames | 어느 정지 장면에서 골랐는지 |
| `schema_version` | integer | `payload` 형식 번호. 형식이 바뀌면 올린다 |
| `payload` | jsonb | 선택 내용 |
| `created_at`, `updated_at` | timestamptz | 다시 입력하면 같은 행을 덮어쓴다 |

사용자는 정지 장면에서 **본인 차량을 드래그로 지목한다.** 드래그한 사각형과 가장 많이 겹치는 탐지(`still_frames.detections`)를 골라 그 누끼를 "내 차량"으로 확정한다. 겹치는 탐지가 없으면 드래그한 사각형을 그대로 쓴다. (D-29)

```json
{
  "drag_bbox": [800, 430, 970, 630],
  "det_id": 1,
...
| 키 | 뜻 |
|---|---|
| `drag_bbox` | 사용자가 그린 사각형. 원본 해상도 기준 픽셀 |
| `det_id` | 맞춰진 탐지 번호. 못 맞췄으면 비어 있다 |
| `bbox` | **확정된 본인 차량 영역.** 2단계 룰이 쓰는 값 |
| `matched` | 탐지와 맞췄으면 `true`, 드래그 사각형을 그대로 썼으면 `false` |

- **본인 차량은 추적하지 않는다.** 주차된 차라 영상 내내 자리가 같으므로 고정된 영역으로 다룬다. 2단계 룰은 "다른 트랙이 이 영역 근처에 왔는가"를 본다. 그래서 `track_id`를 이어 붙일 필요가 없다. (D-29)
- 맞추는 기준(겹침 비율 등)은 🚧 (06장, U-13).
...
확정된 본인 차량 위에서 **파손 부위를 한 번 더 드래그해** 지정한다. (D-29)
...
| `failed` | 여러 번 실패해 DLQ로 옮겨졌다. `error_message`에 마지막 오류가 있다 (D-16) | | ✅ |
...
| 3 | `stills_running` → `stage1_running` | 정지 장면 비식별화가 모두 끝났을 때 | 워커 | `still_frames` 행 생성 |
...
| 13 | 어떤 상태 → `failed` | 작업이 DLQ로 옮겨졌을 때 | 사람 또는 DLQ 처리 코드 | `error_message`. 처리 방법은 🚧 (06장) |
...
- 메시지를 꺼낸 워커는 먼저 4.4의 조건부 UPDATE를 시도한다. 0행이면 아무 일도 하지 않고 메시지를 지운다.
...
- 확장자는 업로드한 파일의 것을 따른다. ⚠️ 허용 확장자·코덱 목록은 05장에서 정한다.
...
CREATE TABLE videos (
  ...
  upload_token_id     bigint      REFERENCES upload_tokens(id) ON DELETE SET NULL,  -- NULL이면 주인이 직접 올림
  s3_key              text        NOT NULL,
  original_filename   text        NOT NULL,
  size_bytes          bigint,                  -- 업로드 완료 알림에서 채운다
...
| 항목 | 어디서 정하나 |
|---|---|
| 비밀번호 해시 방식, 쿠키 속성(`SameSite` 등) | 08장 — 세션 저장 방식은 D-30으로 정리했다(상태 "확정 필요") |
| 업로드 링크 발급 화면·유효 시간 확정, 만료 안내 문구 | 08장, 07장 (D-28) |
| 드래그와 탐지를 맞추는 기준(겹침 비율 등), 누끼를 다각형으로 딸지 사각형만 쓸지 | 06장 (U-13, U-09) (D-29) |
| 룰이 실제로 받는 파손 부위 값(근접 판정 폭, 인접 영역) | 06장 (U-13) → 정해지면 `schema_version`만 올린다 |
| `payload` 검증 규칙 | 05장 (D-25) |
| 청크 길이, 샘플링 fps | U-06, 실측 후 |
| 상태별 화면 문구 | 07장 (U-15) |
| 청크 경계를 넘는 트랙 이어 붙이기 | 06장 |
| DLQ로 간 작업(`failed`)을 사람이 처리하는 방법 | 06장 (D-16 5번) |
